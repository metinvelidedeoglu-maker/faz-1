(() => {
  ...
  function downloadBlob(filename, blob) { const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = filename; link.click(); setTimeout(() => URL.revokeObjectURL(url), 500); }
  ...
  function cellXml(reference, cell, rowStyle = 0) {
    ...
    if (cell.formula) return `<c r="${reference}" s="${style}"><f>${xml(cell.formula)}</f><v>${cell.cached ?? ""}</v></c>`;
    if (cell.type === "number" || cell.type === "date") return `<c r="${reference}" s="${style}"><v>${cell.value ?? ""}</v></c>`;
