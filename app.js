(() => {
  ...
  const state = {
    ...
    search: "",
    month: baseData.monthlySummary?.[baseData.monthlySummary.length - 1]?.month || "Temmuz",
    category: [],
    brand: [],
  ...
  function recalculate(item) {
    ...
    const legacyErmanAdvance = salesDueDays !== null && purchaseDueDays !== null && purchaseDueDays - salesDueDays > 30;
    const ermanAdvanceEnabled = item.ermanAdvanceEnabled === undefined ? legacyErmanAdvance : Boolean(item.ermanAdvanceEnabled);
    const dueAdvantage = purchaseAmount !== null && ermanAdvanceEnabled ? purchaseAmount : 0;
    const ermanAdvance = categoryKey === "metin&erman" && ermanAdvanceEnabled && purchaseAmount !== null ? purchaseAmount / 2 : null;
    return { ...item, category, month, monthKey: monthKeyMap[month] ?? "BELIRSIZ", salesAmount, purchaseAmount, salesDueDays, purchaseDueDays, ermanAdvanceEnabled, grossMarginRate: rate, grossMarginAmount: gross, metinMargin: normalizedMetinMargin, ermanMargin, dueAdvantage, ermanAdvance };
  }
  ...
  function metrics(items = allSales()) {
    ...
    return { sales: sum(items, "salesAmount"), purchase, gross: sum(items, "grossMarginAmount"), rate: purchase ? sum(items, "grossMarginAmount") / purchase : 0, incomplete: items.filter((item) => item.salesAmount === null || item.purchaseAmount === null) };
  }
  ...
  function dashboardView() {
    const items = allSales(), m = metrics(items), summary = monthlySummary(items), topCustomers = grouped(items, "customer").slice(0, 5), topBrands = grouped(items, "brand").slice(0, 6), ermanAdvanceItems = items.filter((item) => num(item.ermanAdvance) > 0), maxCustomer = topCustomers[0]?.sales || 1;
    return `${pageHeading("FAZ 1 / KONTROL", "FAZ 1 satış tablosu", `${baseData.dataPeriod} · ${items.length} kayıt · tüm rakamlar Excel sütunlarından hesaplanır.`, `<button class="button secondary" data-action="export-csv">CSV dışa aktar</button><button class="button primary" data-action="add-sale">＋ Yeni satış</button>`)}
      <section class="hero-panel"><div class="hero-kicker">FAZ 1.xlsx · Sayfa1</div><h2>${compactMoney(m.sales)} satış hacmi</h2><p>Satış, alış ve Metin–Erman marj dağılımı tek tabloda. Kayıt ekleyebilir, düzenleyebilir, silebilir ve aylık Excel çıktısı alabilirsin.</p><div class="hero-meta">Brüt marj<strong>${compactMoney(m.gross)}</strong></div></section>
      ...
      <section class="dashboard-grid"><div class="panel"><div class="panel-heading"><div><h2>Aylık satış performansı</h2><p>Satış tutarı · TL</p></div><button class="panel-link" data-view="sales">Aylık kayıtları aç</button></div><div class="chart-wrap">${chartSvg(summary)}</div></div><div class="panel"><div class="panel-heading"><div><h2>Erman avans</h2><p>İşaretli satışlar</p></div><span class="tag orange">${ermanAdvanceItems.length} kayıt</span></div><div class="alert-list">${ermanAdvanceItems.slice(0, 5).map((item) => `<div class="alert-item"><div class="alert-mark orange">₺</div><div><div class="alert-title">${escapeHtml(item.customer)}</div><div class="alert-text">${compactMoney(item.ermanAdvance)} avans</div></div></div>`).join("") || `<div class="empty-state">Erman avansı işaretli kayıt yok.</div>`}</div></div></section>
  ...
  function salesTable(items) {
    ...
    return `<div class="table-wrap"><table><thead><tr><th>Kategori</th><th>Fatura tarihi</th><th class="customer-column">Müşteri</th><th>Etiket</th><th class="align-right">Satış tutar</th><th class="align-right">Alış tutar</th><th class="align-right">Brüt kâr oranı</th><th class="align-right">Brüt kâr (TL)</th><th class="align-right">Metin marj</th><th class="align-right">Erman marj</th><th class="align-right">Erman avans</th><th>İşlem</th></tr></thead><tbody>${items.map((item) => { const categoryKey = String(item.category ?? "").toLocaleLowerCase("tr-TR"); const categoryClass = categoryKey === "metin" ? "blue" : categoryKey === "metin&erman" ? "purple" : ""; return `<tr><td><span class="tag ${categoryClass}">${escapeHtml(item.category || "Belirsiz")}</span></td><td>${dateText(item.invoiceDate)}</td><td class="customer-column"><strong class="customer-name" title="${escapeHtml(item.customer)}">${escapeHtml(item.customer)}</strong></td><td>${escapeHtml(item.brand || "—")}</td><td class="align-right"><strong>${item.salesAmount === null ? "—" : money(item.salesAmount)}</strong></td><td class="align-right">${item.purchaseAmount === null ? "—" : money(item.purchaseAmount)}</td><td class="align-right">${item.grossMarginRate === null ? "—" : pct(item.grossMarginRate)}</td><td class="align-right">${item.grossMarginAmount === null ? "—" : money(item.grossMarginAmount)}</td><td class="align-right">${item.metinMargin === null ? "—" : money(item.metinMargin)}</td><td class="align-right">${item.ermanMargin === null ? "—" : money(item.ermanMargin)}</td><td class="align-right">${item.ermanAdvance === null ? "—" : money(item.ermanAdvance)}</td><td><div class="row-actions"><button type="button" class="mini-icon edit" data-action="edit-sale" data-id="${escapeHtml(item.id)}" title="Düzenle" aria-label="Düzenle">Düzenle</button><button type="button" class="mini-icon delete" data-action="delete-sale" data-id="${escapeHtml(item.id)}" title="Sil" aria-label="Sil">×</button></div></td></tr>`; }).join("")}</tbody></table></div>`;
  }
  ...
  function exportCsv() { const headers = ["KATEGORİ", "FATURA TARİHİ", "MÜŞTERİ", "ETİKET", "SATIŞ TUTAR", "ALIŞ TUTAR", "BRÜT KÂR ORANI", "BRÜT KÂR (TL)", "METİN MARJ", "ERMAN MARJ", "ERMAN AVANS"]; const rows = filteredSales().map((item) => [item.category, item.invoiceDate, item.customer, item.brand, item.salesAmount, item.purchaseAmount, item.grossMarginRate, item.grossMarginAmount, item.metinMargin, item.ermanMargin, item.ermanAdvance].map(csvCell).join(";")); downloadText(`FAZ1-${state.month}-2026.csv`, `\uFEFF${headers.map(csvCell).join(";")}\n${rows.join("\n")}`, "text/csv;charset=utf-8"); showToast(`${state.month} kayıtları CSV’ye aktarıldı.`); }
  ...
  function makeXlsx(items, month) {
    const headers = ["KATEGORİ", "FATURA TARİHİ", "MÜŞTERİ", "ETİKET", "SATIŞ TUTAR", "ALIŞ TUTAR", "BRÜT KÂR ORANI", "BRÜT KÂR (TL)", "METİN MARJ", "ERMAN MARJ", "ERMAN AVANS"];
    const tableRows = [headers.map((value) => ({ value, style: 1 }))];
    items.forEach((item, index) => { const row = index + 2; tableRows.push([
      { value: item.category ?? "", type: "string" }, { value: excelSerial(item.invoiceDate), type: "date", style: 3 }, { value: item.customer, type: "string" }, { value: item.brand, type: "string" }, { value: item.salesAmount, type: "number", style: 2 }, { value: item.purchaseAmount, type: "number", style: 2 }, { formula: `IFERROR(E${row}/F${row}-1,"")`, cached: item.grossMarginRate, style: 4 }, { formula: `IFERROR(E${row}-F${row},"")`, cached: item.grossMarginAmount, style: 2 }, { formula: `IF(LOWER(A${row})="metin",H${row},IF(LOWER(A${row})="metin&erman",H${row}/2,""))`, cached: item.metinMargin, style: 2 }, { formula: `IF(LOWER(A${row})="metin&erman",H${row}/2,"")`, cached: item.ermanMargin, style: 2 }, { value: item.ermanAdvance, type: "number", style: 2 },
    ]); });
    const last = Math.max(items.length + 1, 2), summaryRows = [[{ value: `FAZ 1.xlsx · ${month} 2026 Aylık Özet`, style: 1 }], [], [{ value: "Kayıt sayısı" }, { formula: `COUNTA('İşlemler'!C2:C${last})`, cached: items.length }], [{ value: "Satış toplamı" }, { formula: `SUM('İşlemler'!E2:E${last})`, cached: sum(items, "salesAmount"), style: 2 }], [{ value: "Alış toplamı" }, { formula: `SUM('İşlemler'!F2:F${last})`, cached: sum(items, "purchaseAmount"), style: 2 }], [{ value: "Brüt kâr" }, { formula: `SUM('İşlemler'!H2:H${last})`, cached: sum(items, "grossMarginAmount"), style: 2 }], [{ value: "Brüt kâr oranı" }, { formula: "IFERROR(B6/B5,0)", cached: metrics(items).rate, style: 4 }], [{ value: "Metin marj" }, { formula: `SUM('İşlemler'!I2:I${last})`, cached: sum(items, "metinMargin"), style: 2 }], [{ value: "Erman marj" }, { formula: `SUM('İşlemler'!J2:J${last})`, cached: sum(items, "ermanMargin"), style: 2 }], [{ value: "Erman avans" }, { formula: `SUM('İşlemler'!K2:K${last})`, cached: sum(items, "ermanAdvance"), style: 2 }], [], [{ value: "Etiket", style: 1 }, { value: "Kayıt", style: 1 }, { value: "Satış", style: 1 }, { value: "Brüt kâr", style: 1 }]];
    const brands = grouped(items, "brand"); brands.forEach((brand) => { const row = summaryRows.length + 1; summaryRows.push([{ value: brand.name }, { formula: `COUNTIF('İşlemler'!$D$2:$D$${last},A${row})`, cached: brand.count }, { formula: `SUMIF('İşlemler'!$D$2:$D$${last},A${row},'İşlemler'!$E$2:$E$${last})`, cached: brand.sales, style: 2 }, { formula: `SUMIF('İşlemler'!$D$2:$D$${last},A${row},'İşlemler'!$H$2:$H$${last})`, cached: brand.gross, style: 2 }]); });
    const files = { "[Content_Types].xml": `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/><Override PartName="/xl/workbook.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml"/><Override PartName="/xl/worksheets/sheet1.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/><Override PartName="/xl/worksheets/sheet2.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/><Override PartName="/xl/styles.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.styles+xml"/></Types>`, "_rels/.rels": `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/></Relationships>`, "xl/workbook.xml": `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><workbookPr/><calcPr calcId="191029" calcMode="auto" fullCalcOnLoad="1" forceFullCalc="1"/><sheets><sheet name="Özet" sheetId="1" r:id="rId1"/><sheet name="İşlemler" sheetId="2" r:id="rId2"/></sheets></workbook>`, "xl/_rels/workbook.xml.rels": `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/><Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet2.xml"/><Relationship Id="rId3" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/></Relationships>`, "xl/styles.xml": stylesXml(), "xl/worksheets/sheet1.xml": sheetXml(summaryRows, [32, 18, 19, 20], "D"), "xl/worksheets/sheet2.xml": sheetXml(tableRows, [18, 14, 26, 18, 16, 16, 17, 17, 16, 16, 16], "K") };
